...
const Graph: React.FC<GraphProps> = ({ investOptions, userInfo }) => {
  ...
  const getInvestmentChartOption = (data: Data, mode: 'yearly' | 'cumulative') => {
    ...
    const totalSeries = {
      ...
      type: 'line',
      data: data.sumOfFortunePerYear.map((item) => (mode === 'cumulative' ? item.totalInvestment : item.investment)),
      lineStyle: {
        width: 3,
      },
      emphasis: {
          ...
          params.forEach((item: any) => {
            if (item.value !== null) {
              ...
              if (displayMode === 'rate') {
                ...
                valueDisplay = item.value.toLocaleString(undefined, {
    ...
    <>
      ...
      {data && (
        <Space direction="vertical" style={{ width: '100%', marginTop: '20px' }}>
          <Title level={4} id="chart-display">
            图表展示
          </Title>
            ...
            <Tabs.TabPane tab="利润分析" key="3">
              <Card>
                <Space direction="vertical" style={{ width: '100%', marginBottom: '20px' }}>
                  ...
                  <Radio.Group value={profitDisplayMode} onChange={(e) => setProfitDisplayMode(e.target.value)}>
            ...
            {data.sumOfFortunePerYear.map((item) => {
              ...
              return (
                <Col key={item.year} span={8}>
                  <Text>
                    {item.year}年{age !== null ? ` (${age}岁)` : ''}: ¥
                    {item.fortune.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </Text>
                </Col>
              )
            })}
          ...
          <Title level={4} id="investment-details">
            各投资详情
          </Title>

          {data.OptionData.map((option) => (
            <div key={option.id}>
              <Title level={5} id={`investment-detail-${option.id}`}>
                {option.name || '未命名投资'}
              </Title>
              <Row gutter={[16, 16]} style={{ marginBottom: '20px' }}>
                <Col span={8}>
                  <Card title="总资产">
                    <Text>
                      ¥
                      {option.fortunePerYear[option.fortunePerYear.length - 1].fortune.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </Text>
                ...
                <Col span={8}>
                  <Card title="总投入">
                    <Text>
                      ¥
                      {option.totalInvestment.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </Text>
                ...
                <Col span={8}>
                  <Card title="总利润">
                    <Text>
                      ¥
                      {option.totalProfit.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </Text>
